...
// Clientes ativos por SaaS
// RPM: Carbon (Marcondes) descartado 2026-05-02, sem cliente ativo no momento
// EDR: Jackson Alcantara + Rodrigues Construtora removidos 2026-05-03 (só EDR Engenharia)
...
// ═══ MONTAGEM ═══
// RPM Pro removido do relatorio diario até ter cliente ativo novamente
// (Carbon/Marcondes descartado em 2026-05-02 — trial expirado, nunca pagou)
async function gerarResumo() {
  const j = janelaOntem()

  const [edr, dmtech] = await Promise.all([
    resumoEDR(j),
    resumoDMTech(j),
  ])
